  ...
  case actual of
    ...
    Just value ->
      unless (approxEqual expected value) $
        assertFailure $ label ++ ": ожидалось " ++ show expected ++ ", получили " ++ show value
...
assertApproxPair label (ex, ey) (ax, ay) = do
  unless (approxEqual ex ax) $
    assertFailure $ label ++ " по x: ожидалось " ++ show ex ++ ", получили " ++ show ax
  unless (approxEqual ey ay) $
    assertFailure $ label ++ " по y: ожидалось " ++ show ey ++ ", получили " ++ show ay
...
simultaneousTest = TestLabel "simultaneous algorithms" $ TestCase $ do
  ...
  assertEqual "линейный алгоритм выдал нужное число значений"
    (length linearExpected) (length linearActual)
  assertEqual "алгоритм Ньютона выдал нужное число значений"
    (length newtonExpected) (length newtonActual)
